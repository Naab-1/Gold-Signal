...
of M15/H1 data). The real run's result is reported in full below, and
...
2. **Every configuration tested is recorded.** `OptimizationResult.trials`
   ...
   summary, so a full audit trail survives even for rejected
...
selection (though still recorded), so a configuration can't be judged
...
**Grid**: `trend_strength_atr_multiple` in {0.75, 1.0 (baseline), 1.25}
combined with `max_extension_atr_multiple` in {1.0 (baseline), 1.5
(baseline), 2.0}: 2 fields, bounded to their immediate neighbors either
side of the existing default, 5 trials total (baseline + 4
combinations).
...
clear both the complexity penalty (2 changed parameters times 0.02 = 0.04)
...
- **This is the first real-history run for any Phase 4 candidate in
  ...
  instruments, or establish that Trend Pullback has a real edge, the
  same caution every prior phase in this program has applied to its
  own real-data findings.
