...
# Trend Pullback (Phase 4, candidate Family A -- docs/phase4_trend_pullback.md):
# only the generic/shared half of its config (timeframes, indicator
# periods, ATR-stop multiplier, cost estimates) lives in a ModeConfig here,
# loaded independently via load_trend_pullback_mode_config so this family
# is separately tunable rather than silently sharing Day-Trade's values.
# Its own family-specific thresholds (pullback RSI levels, extension
# limit, etc.) live in TrendPullbackConfig
# (strategy/candidates/trend_pullback.py), not here -- fields below that
# this family doesn't actually read (RSI thresholds, structure_lookback,
# chop filter, continuation_*, its own min_net_reward_r) still need valid
# defaults purely because load_mode_config populates the whole ModeConfig
# dataclass; TrendPullbackStrategy simply never reads them.
_TRENDPULLBACK_MODE_DEFAULTS: dict[str, str] = {
    "ENABLED": "true",
    "ENTRY_TIMEFRAME": "M15",
    "CONFIRMATION_TIMEFRAME": "H1",
    "EMA_FAST_PERIOD": "20",
...
# Breakout Continuation (Phase 4, candidate Family B --
# docs/phase4_breakout_continuation.md): same split as Trend Pullback --
# only the generic/shared half (timeframes, indicator periods, ATR-stop
# multiplier, cost estimates) lives here; family-specific thresholds
# (breakout/confirmation shape, structure lookback, cooldown, etc.) live
# in BreakoutContinuationConfig (strategy/candidates/breakout_continuation.py).
_BREAKOUTCONTINUATION_MODE_DEFAULTS: dict[str, str] = {
    "ENABLED": "true",
    "ENTRY_TIMEFRAME": "M5",
    "CONFIRMATION_TIMEFRAME": "M15",
    "EMA_FAST_PERIOD": "20",
...
# Range Rejection (Phase 4, candidate Family D --
# docs/phase4_range_rejection.md): same split as Families A/B/C -- only
# the generic/shared half lives here; family-specific thresholds
# (range width, the "not trending" ceiling, rejection shape) live in
# RangeRejectionConfig (strategy/candidates/range_rejection.py).
_RANGEREJECTION_MODE_DEFAULTS: dict[str, str] = {
    "ENABLED": "true",
    "ENTRY_TIMEFRAME": "M15",
    "CONFIRMATION_TIMEFRAME": "H1",
    "EMA_FAST_PERIOD": "20",
...
def load_trend_pullback_mode_config(env: Mapping[str, str] | None = None) -> ModeConfig:
    """The generic/shared half of Trend Pullback's config (Phase 4, Family
    A) -- see `_TRENDPULLBACK_MODE_DEFAULTS`'s module comment. Its own
    family-specific thresholds are `TrendPullbackConfig`
    (`strategy/candidates/trend_pullback.py`), loaded separately.
    """
    return load_mode_config("TRENDPULLBACK_", _TRENDPULLBACK_MODE_DEFAULTS, env)


def load_breakout_continuation_mode_config(env: Mapping[str, str] | None = None) -> ModeConfig:
    """The generic/shared half of Breakout Continuation's config (Phase 4,
    Family B) -- see `_BREAKOUTCONTINUATION_MODE_DEFAULTS`'s module
    comment. Its own family-specific thresholds are
...
def load_range_rejection_mode_config(env: Mapping[str, str] | None = None) -> ModeConfig:
    """The generic/shared half of Range Rejection's config (Phase 4,
    Family D) -- see `_RANGEREJECTION_MODE_DEFAULTS`'s module comment.
    Its own family-specific thresholds are `RangeRejectionConfig`
